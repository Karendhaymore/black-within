...
def _auto_migrate_profiles_table():
    with engine.begin() as conn:
        ...
        conn.execute(text("ALTER TABLE profiles ALTER COLUMN identity_preview TYPE TEXT;"))
...
for fn, label in [
    ...
    (_auto_migrate_admin_messages_table, "AUTO_MIGRATE_ADMIN_MESSAGES"),
    (_auto_migrate_blocked_users_table, "AUTO_MIGRATE_BLOCKED_USERS"),
...
def upsert_my_profile(payload: UpsertMyProfilePayload):
    ...
    with Session(engine) as session:
        ...
        if existing:
            ...
            existing.mate_qualities = payload.mateQualities
            existing.fun_activities = payload.funActivities
            existing.smokes = payload.smokes
            existing.drinks = payload.drinks
            existing.education_level = payload.educationLevel
            ...
            new_profile = Profile(
                ...
                photo2_position_x=photo2_position_x,
                photo2_position_y=photo2_position_y,
                ...
                mate_qualities=payload.mateQualities,
                fun_activities=payload.funActivities,
...
def admin_resolve_report(
    ...
    with engine.begin() as conn:
        ...
        # ✅ IMPORTANT: cast :status to VARCHAR everywhere it is used
        if note_col:
            sql = text(
                f"""
                UPDATE reports
                SET status = CAST(:status AS VARCHAR),
                    {note_col} = :note,
                    resolved_at = CASE
                        WHEN CAST(:status AS VARCHAR) = 'resolved' THEN NOW()
                        ELSE NULL
                    END
                WHERE id = :id
                """
